...
export const CustomNode = memo(function CustomNode({ id, type, data, selected }: CustomNodeProps) {
  ...
  return (
    <div
      ...
      className={`${colors.bg} ${colors.border} border-2 rounded-lg shadow-sm w-[180px] transition-all duration-200 cursor-pointer ${selected ? 'ring-2 ring-blue-400 ring-offset-1 shadow-md' : ''}`}
    >
        ...
        {nodeType === 'server' ? (
          <>
            ...
            <p className="text-xs text-gray-500 truncate flex items-center gap-1">
              ...
              {(data as ServerData).ssl && <Lock size={10} className="text-gray-400 shrink-0" />}
            </p>
          </>
        ) : nodeType === 'location' ? (
          <>
            <p className="text-sm font-medium text-gray-800 truncate">{(data as LocationData).path}</p>
            {(data as LocationData).mode === 'proxy' && (data as LocationData).proxyPass && (
              <p className="text-xs text-gray-500 truncate">→ {(data as LocationData).proxyPass}</p>
            )}
            {(data as LocationData).mode === 'static' && (data as LocationData).root && (
              <p className="text-xs text-gray-500 truncate">{(data as LocationData).root}</p>
            )}
            {(data as LocationData).mode === 'block' && (
              <p className="text-xs text-red-500 truncate">禁止访问 ({(data as LocationData).blockStatus || 403})</p>
            )}
          ...
          <>
            <p className="text-sm font-medium text-gray-800 truncate">{(data as BackendData).address}</p>
            <p className="text-xs text-gray-500 truncate">
              {[
          ...
          <>
            <p className="text-xs text-gray-500 truncate">http://{(data as RedirectData).fromDomain}</p>
            <p className="text-xs text-gray-500 truncate">→ {(data as RedirectData).toUrl}</p>
          </>
        ) : nodeType === 'static' ? (
          <>
            <p className="text-sm font-medium text-gray-800 truncate">{(data as StaticData).path}</p>
            <p className="text-xs text-gray-500 truncate">{(data as StaticData).root}</p>
            {(data as StaticData).expires && (
              <p className="text-xs text-gray-400 truncate">缓存 {(data as StaticData).expires}</p>
            )}
          </>
        ) : (
          <p className="text-xs text-gray-500">未知类型</p>
        )}
      </div>
      <Handle type="target" position={Position.Top} id="top" className="!w-2.5 !h-2.5 !bg-gray-400 !border-2 !border-white" />
      <Handle type="source" position={Position.Bottom} id="bottom" className="!w-2.5 !h-2.5 !bg-gray-400 !border-2 !border-white" />
