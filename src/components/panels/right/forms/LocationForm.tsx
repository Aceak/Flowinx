...
import { Lightbulb, ArrowRightLeft, Folder, ShieldBan, CornerDownRight } from 'lucide-react';
...
function isCustomPath(path: string) {
  return !PATH_OPTIONS.slice(0, -1).some((o) => o.value === path);
}
function isCustomProxy(proxy: string) {
  return !PROXY_PRESETS.slice(0, -1).some((o) => o.value === proxy);
}

export function LocationForm({ data, onChange }: Props) {
  const [showAccess, setShowAccess] = useState(false);
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [customPath, setCustomPath] = useState(() => isCustomPath(data.path));
  const [customProxy, setCustomProxy] = useState(() => isCustomProxy(data.proxyPass));

  // 节点切换时同步自定义路径状态
  useEffect(() => { setCustomPath(isCustomPath(data.path)); }, [data.path]);
  useEffect(() => { setCustomProxy(isCustomProxy(data.proxyPass)); }, [data.proxyPass]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <div className="flex flex-col gap-1">
          {PATH_OPTIONS.map((opt) => (
            <label key={opt.value}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-all text-sm ${(opt.value === '__custom__' && customPath) || data.path === opt.value
      ...
      {/* 模式选择 */}
      <div>
        <span className="text-gray-600 dark:text-neutral-400 text-sm font-medium mb-2 block">策略</span>
        <div className="grid grid-cols-2 gap-2">
          <button type="button"
            onClick={() => onChange({ mode: 'static', proxyPass: '' })}
            className={`py-2.5 px-2 rounded-lg border-2 text-sm font-medium transition-all flex flex-col items-center ${mode === 'static' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 shadow-sm' : 'border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-gray-500 dark:text-neutral-400 hover:border-gray-300 dark:hover:border-neutral-500'}`}>
          ...
          <button type="button"
            ...
            className={`py-2.5 px-2 rounded-lg border-2 text-sm font-medium transition-all flex flex-col items-center ${mode === 'proxy' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 shadow-sm' : 'border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-gray-500 dark:text-neutral-400 hover:border-gray-300 dark:hover:border-neutral-500'}`}>
          ...
          <button type="button"
            onClick={() => onChange({ mode: 'redirect', proxyPass: '' })}
            className={`py-2.5 px-2 rounded-lg border-2 text-sm font-medium transition-all flex flex-col items-center ${mode === 'redirect' ? 'border-cyan-500 bg-cyan-50 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 shadow-sm' : 'border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-gray-500 dark:text-neutral-400 hover:border-gray-300 dark:hover:border-neutral-500'}`}>
            <CornerDownRight size={16} className="mb-0.5" />重定向<span className="text-xs font-normal text-gray-400 dark:text-neutral-500">跳转URL</span>
          </button>
          <button type="button"
            onClick={() => onChange({ mode: 'block', proxyPass: '' })}
            className={`py-2.5 px-2 rounded-lg border-2 text-sm font-medium transition-all flex flex-col items-center ${mode === 'block' ? 'border-red-500 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 shadow-sm' : 'border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-gray-500 dark:text-neutral-400 hover:border-gray-300 dark:hover:border-neutral-500'}`}>
            ...
            <div>
              ...
              <div className="flex flex-col gap-1">
                {PROXY_PRESETS.map((opt) => (
                  <label key={opt.value}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-all text-sm ${(opt.value === '__custom__' && customProxy) || data.proxyPass === opt.value
                        ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'
                        : 'border-gray-200 dark:border-neutral-600 hover:border-gray-300 dark:hover:border-neutral-500 text-gray-600 dark:text-neutral-400'
                      }`}
                  >
        ...
        {mode === 'redirect' && (
          <>
            <label className="flex flex-col gap-1">
              <span className="text-gray-600 dark:text-neutral-400 text-sm font-medium">跳转到哪个 URL</span>
              <input type="text" value={data.redirectUrl} className={INPUT_CLASS}
                placeholder="https://example.com" onChange={(e) => onChange({ redirectUrl: e.target.value })} />
              <span className="text-xs text-gray-400 dark:text-neutral-500">支持 $scheme、$host 等 nginx 变量</span>
            </label>
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" checked={data.redirectPermanent} className={CHECKBOX_CLASS}
                onChange={(e) => onChange({ redirectPermanent: e.target.checked })} />
              <span className="text-gray-700 dark:text-neutral-300">永久重定向 (301)</span>
              <span className="text-xs text-gray-400 dark:text-neutral-500">关闭则为临时重定向 (302)</span>
            </label>
          </>
        )}
        ...
        {mode === 'block' && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {[
                ...
                <button key={code} type="button"
                  ...
                  className={`py-2.5 px-2 rounded-lg border-2 text-sm transition-all text-left ${data.blockStatus === code ? 'border-red-500 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 shadow-sm' : 'border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-gray-500 dark:text-neutral-400 hover:border-gray-300 dark:hover:border-neutral-500'}`}>
                  <span className="font-medium block">{label}</span>
                  <span className="text-xs text-gray-400 dark:text-neutral-500">{desc}</span>
                </button>
              ))}
              {/* 自定义状态码 */}
              <div className={`py-2 px-2 rounded-lg border-2 text-sm transition-all col-span-2 flex items-center gap-2 ${![403, 404, 401, 444, 405, 410, 502, 503].includes(data.blockStatus) ? 'border-red-500 bg-red-50 dark:bg-red-900/30' : 'border-gray-200 dark:border-neutral-600 bg-white dark:bg-neutral-800'
                }`}>
            ...
            <label className="flex flex-col gap-1 mt-2">
              <span className="text-gray-500 dark:text-neutral-400 text-xs">自定义错误页面路径（可选）</span>
              <input type="text" value={data.blockPage || ''} className={INPUT_CLASS}
                placeholder="/403.html"
                onChange={(e) => onChange({ blockPage: e.target.value })} />
              <span className="text-xs text-gray-400 dark:text-neutral-500">留空则直接返回状态码</span>
            </label>
          </div>
        )}
